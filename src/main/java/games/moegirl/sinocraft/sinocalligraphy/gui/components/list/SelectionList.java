...
public abstract class SelectionList<T> extends AbstractContainerEventHandler implements Widget, NarratableEntry {
    ...
    public SelectionList<T> resize(int left, int top) {
        return setSize(left, top, x, y, width, height);
    }

    public SelectionList<T> setSize(int left, int top, int x, int y, int width, int height) {
        this.width = width;
        this.height = height;
        this.x = x;
        this.y = y;
    ...
    public SelectionList<T> setLayout(SelectionLayout<T> layout) {
        this.layout = layout;
        return this;
    }
    ...
    protected abstract void drawCanvas(List<SelectionEntry<T>> displayItems, PoseStack poseStack, int mouseX, int mouseY, float partialTick);
